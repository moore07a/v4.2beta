...
const ALLOWLIST_DOMAINS  = (process.env.ALLOWLIST_DOMAINS  || "righttodreamdk.com,ninebbf037657fae97.onrender.com")
  .split(",").map(s=>s.trim()).filter(Boolean);
const ALLOWLIST_SUFFIXES = (process.env.ALLOWLIST_SUFFIXES || ".netlify.app")
  .split(",").map(s=>s.trim()).filter(Boolean);
