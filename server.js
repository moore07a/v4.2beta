...
// === ADD THIS NEW DEBUG ROUTE RIGHT AFTER THE EXISTING ONE ===
app.get("/__debug/decrypt-manual", requireAdmin, (req, res) => {
  const data = "MMpPGNj0FmFrCjDboOpO7MQc2Z5-D-23_AKC6JP1b8EovfQ6GsMWZZ3hIIZSDKSAjmhSmcfgEiWXU5p";
  
  // Remove leading underscore if present
  const cleanData = data.startsWith("_") ? data.slice(1) : data;
  
  // Convert from base64url
  const normalized = cleanData.replace(/-/g, '+').replace(/_/g, '/');
  const padded = normalized + '='.repeat((4 - normalized.length % 4) % 4);
  const buffer = Buffer.from(padded, 'base64');
  
  console.log("Manual decryption attempt:");
  console.log("Input length:", cleanData.length, "chars");
  console.log("Decoded length:", buffer.length, "bytes");
  
  // Try both IV sizes
  for (const ivLen of [12, 16]) {
    if (buffer.length > ivLen + 16) {
      const iv = buffer.slice(0, ivLen);
      const ct = buffer.slice(ivLen, -16);
      ...
      try {
        const result = gcmDecryptAnyKey(iv, ct, tag);
        if (result.buf) {
          const decrypted = result.buf.toString('utf8');
          console.log("✅ SUCCESS! Decrypted:", decrypted);
          return res.json({ success: true, url: decrypted, ivLength: ivLen, keyIndex: result.keyIndex });
        } else {
          console.log("❌ Decryption failed, key index:", result.keyIndex);
